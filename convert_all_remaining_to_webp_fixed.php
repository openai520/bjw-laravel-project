...
// 使用Laravel模型查询需要转换的产品
$need_conversion_products = Product::whereHas('images', function($q) {
    $q->where('image_path', 'not like', '%.webp')
      ->where('image_path', 'not like', '%placeholder%')
      ->where('image_path', '!=', '')
      ->whereNotNull('image_path');
})->count();

log_message("需要转换的产品总数: {$need_conversion_products}");

$processed_products = 0;
$offset = 0;

while ($processed_products < $need_conversion_products) {
    ...
    $products = Product::whereHas('images', function($q) {
        $q->where('image_path', 'not like', '%.webp')
          ...
          ->where('image_path', '!=', '')
          ->whereNotNull('image_path');
    })->offset($offset)->limit($batch_size)->get(['id', 'name']);
    ...
    foreach ($products as $product) {
        try {
            ...
            foreach ($images as $image) {
                ...
                if ($result['success']) {
                    // 更新数据库中的图片路径
                    $image->image_path = $webp_relative_path;
                    $image->save();
        ...
        if ($processed_products % 10 == 0) {
            log_message("\n📊 进度报告:");
            log_message("已处理产品: {$processed_products}/{$need_conversion_products}");
