        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex space-x-2">
                <form method="GET" class="flex items-center space-x-3">
                    <select name="days" class="rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                    ...
                    <div class="flex-shrink-0">
                        <div class="w-10 h-10 bg-yellow-100 rounded-lg flex items-center justify-center">
                            <svg class="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2V9z"></path>
                            </svg>
                        </div>
                    </div>
                    <div class="ml-4 flex-1">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="w-48 bg-gray-200 rounded-full h-3">
                                    <div class="bg-blue-600 h-3 rounded-full transition-all duration-300" style="width: {{ $stat['count'] > 0 ? ($stat['count'] / $maxCount) * 100 : 0 }}%"></div>
                                </div>
                            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    @forelse($popularProducts as $index => $item)
                        <div class="flex items-center justify-between p-4 {{ $index < 3 ? 'bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-lg' : 'border-b border-gray-100 last:border-0' }}">
                            <div class="flex items-center space-x-3">
                                <div class="flex-shrink-0">
                                    <span class="inline-flex items-center justify-center h-8 w-8 rounded-full {{ $index === 0 ? 'bg-yellow-400 text-white' : ($index === 1 ? 'bg-gray-400 text-white' : ($index === 2 ? 'bg-orange-400 text-white' : 'bg-gray-200 text-gray-600')) }} text-sm font-bold">
                                        {{ $index + 1 }}
                                    </span>
                                </div>
                ...
                @if($popularProducts->count() > 0)
                    <div class="mt-6 text-center">
                        <a href="{{ route('admin.products.index') }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">
                            查看所有产品 →
                        </a>
                    </div>
                @endif
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">总访问</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">今日访问</th>
                        ...
                        @forelse($popularProducts as $index => $item)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <span class="inline-flex items-center justify-center h-6 w-6 rounded-full {{ $index < 3 ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-600' }} text-xs font-medium">
                                        {{ $index + 1 }}
                                    </span>
                                </td>
                                    ...
                                    <div class="flex items-center">
                                        <div>
                                            ...
                                            <div class="text-sm text-gray-500">
                                                ID: {{ $item->product_id }} | ${{ number_format($item->product->price ?? 0, 2) }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm font-semibold text-gray-900">{{ $item->product ? $item->product->today_view_count : 0 }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($item->product)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $item->product->status === 'published' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
