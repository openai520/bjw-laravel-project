...
<div class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('admin.home_settings.index') }}" class="text-sm text-gray-600 hover:text-gray-800">&larr; 返回列表</a>
        </div>
        ...
        <form action="{{ route('admin.home_settings.update_featured_products', $category) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            @method('PUT')
                ...
                @for ($i = 1; $i <= 5; $i++)
                    <div class="p-4 border rounded-md product-slot" data-slot-id="{{ $i }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                            <div>
                                <label for="product_search_{{ $i }}" class="block text-sm font-medium text-gray-700">搜索产品</label>
                                ...
                                <label for="product_id_{{ $i }}" class="block text-sm font-medium text-gray-700 sr-only">选择产品</label>
                                <select name="products[{{ $i-1 }}][product_id]" id="product_id_{{ $i }}" 
                            ...
                            <div>
                                <label for="display_order_{{ $i }}" class="block text-sm font-medium text-gray-700">显示顺序 (1-5)</label>
                                <input type="number" name="products[{{ $i-1 }}][display_order]" id="display_order_{{ $i }}" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    productSlots.forEach(slot => {
        ...
        searchInput.addEventListener('input', function() {
            ...
            if (productSelector.selectedIndex !== -1 && productSelector.options[productSelector.selectedIndex].style.display === 'none') {
        ...
        productSlots.forEach(slot => {
            ...
            if (productId) {
                if (selectedProducts.includes(productId)) {
                    alert('错误：产品 "' + slot.querySelector('.product-selector option:checked').dataset.productName + '" 被重复选择。');
                    hasError = true;
                }
