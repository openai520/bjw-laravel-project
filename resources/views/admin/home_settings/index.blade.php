                        ...
                        @forelse($categories as $category)
                            <tr>
                                ...
                                <td class="px-6 py-4">
                                    @if ($category->homeFeaturedProducts->isNotEmpty())
                                        <ul class="list-disc list-inside text-sm text-gray-700 space-y-1">
                                            @foreach ($category->homeFeaturedProducts->sortBy('display_order') as $featuredProduct)
                                                <li>
                                                    <span
                                                        class="font-medium">{{ $featuredProduct->product->name ?? '[产品信息丢失]' }}</span>
                                    ...
                                    @else
                                        <span
                                            class="text-sm text-gray-400 italic">{{ __('admin.no_featured_products') }}</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('admin.home_settings.edit_featured_products', $category) }}"
