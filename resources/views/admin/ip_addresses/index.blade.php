                        ...
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    {{ __('admin.country') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    {{ __('admin.visit_count') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    {{ __('admin.last_visit') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    {{ __('admin.actions') }}</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($ipAddresses as $ip)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $ip->visit_count }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $ip->last_visit }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        @if (in_array($ip->ip_address, $blockedIps))
                                            <form action="{{ route('admin.ip_addresses.unblock', $ip->ip_address) }}"
                                        ...
                                        @else
                                            <form action="{{ route('admin.ip_addresses.block') }}" method="POST"
                                                class="inline">
                                                @csrf
                                                <input type="hidden" name="ip_address" value="{{ $ip->ip_address }}">
                                                <button type="submit"
                                                    class="text-white bg-red-500 hover:bg-red-600 px-3 py-1 rounded text-xs"
