...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    @if($successCount > 0 || $failureCount > 0)
        <div class="mb-6 p-4 rounded {{ $failureCount > 0 ? 'bg-yellow-100 border border-yellow-400 text-yellow-700' : 'bg-green-100 border border-green-400 text-green-700' }}">
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div class="flex flex-wrap items-end gap-x-4 gap-y-2 mb-4 border-b pb-4">
                <div class="flex-shrink-0">
                    ...
                    <select id="bulk-category" class="form-select rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                ...
                <div class="flex-shrink-0">
                    ...
                    <input type="number" min="1" max="5" id="bulk-category-from" placeholder="1" class="form-input rounded-md border-gray-300 shadow-sm w-20 text-sm">
                </div>
                <div class="flex-shrink-0">
                    <label for="bulk-category-to" class="block text-sm font-medium text-gray-700 mb-1">到行</label>
                    <input type="number" min="1" max="5" id="bulk-category-to" placeholder="5" class="form-input rounded-md border-gray-300 shadow-sm w-20 text-sm">
                </div>
                <div class="self-end">
                    <button type="button" id="apply-bulk-category" class="bg-gray-600 hover:bg-gray-700 text-white py-1.5 px-3 rounded text-sm">应用分类</button>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-end gap-x-4 gap-y-2">
                <div class="flex-shrink-0">
                    ...
                    <input type="number" min="1" id="bulk-min-qty" class="form-input rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                </div>
                <div class="flex-shrink-0">
                    <label for="bulk-min-qty-from" class="block text-sm font-medium text-gray-700 mb-1">从行</label>
                    <input type="number" min="1" max="5" id="bulk-min-qty-from" placeholder="1" class="form-input rounded-md border-gray-300 shadow-sm w-20 text-sm">
                </div>
                <div class="flex-shrink-0">
                    <label for="bulk-min-qty-to" class="block text-sm font-medium text-gray-700 mb-1">到行</label>
                    <input type="number" min="1" max="5" id="bulk-min-qty-to" placeholder="5" class="form-input rounded-md border-gray-300 shadow-sm w-20 text-sm">
                </div>
                <div class="self-end">
                    <button type="button" id="apply-bulk-min-qty" class="bg-gray-600 hover:bg-gray-700 text-white py-1.5 px-3 rounded text-sm">应用数量</button>
                </div>
            </div>
    ...
    <div class="flex justify-end mb-6">
        <button type="submit" form="batch-upload-form" id="submit-button" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded">
            提交全部产品
        </button>
    </div>
    ...
    <!-- 主表单 -->
    <form action="{{ route('admin.products.batch_import.handle') }}" method="POST" id="batch-upload-form" class="bg-white p-6 rounded-lg shadow">
        @csrf
        ...
        <!-- 产品行 -->
        @for ($i = 0; $i < 5; $i++)
        <div class="grid grid-cols-12 gap-4 mb-3 items-start py-3 {{ $i % 2 == 0 ? 'bg-gray-50' : 'bg-white' }} rounded {{ isset($importErrorMessages['row_'.($i+1)]) ? 'border border-red-300' : '' }}">
            ...
            <div class="col-span-2">
                <div class="relative">
                    ...
                    <div x-show="nameError"
                         x-text="nameError"
                         class="absolute top-full left-0 text-red-500 text-xs mt-1 bg-white p-1 rounded shadow-sm"></div>
                </div>
            </div>
            ...
            <div class="col-span-1">
                <input type="number" step="0.01" min="0" name="products[{{ $i }}][price]" placeholder="价格 (必填)"
                    class="form-input w-full rounded border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 text-sm">
            </div>

            <div class="col-span-1">
                <input type="number" min="1" name="products[{{ $i }}][min_order_quantity]" placeholder="最小订购量"
                    class="form-input w-full rounded border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 text-sm">
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    pondInputs.forEach(input => {
                ...
                process: {
                    ...
                    onerror: (response) => {
                        ...
                        try {
                            ...
                            if (errorData && errorData.errors && errorData.errors.filepond) {
        ...
        pond.on('processfile', (error, file) => {
            ...
            if (!container) {
                 console.error(`[JS ProcessFile] Row ${rowIndex}: Container not found! ID: temp_images_container_${rowIndex}`);
                 return;
            }
            ...
            if (!existingInput) {
                ...
                console.log(`[JS ProcessFile] Row ${rowIndex}: Added hidden input for ID ${fileId}`);
            }
        });
        ...
        pond.on('removefile', (error, file) => {
            ...
            if (!fileId) {
                console.warn('[JS RemoveFile] No serverId found for removed file, cannot update hidden input.');
                return;
            }
            ...
            const container = document.getElementById(`temp_images_container_${rowIndex}`);
             if (!container) {
                 console.error(`[JS RemoveFile] Row ${rowIndex}: Container not found! ID: temp_images_container_${rowIndex}`);
                 return;
            }

            // 查找并移除对应的隐藏输入框
            const inputToRemove = container.querySelector(`input[name="products[${rowIndex}][temp_image_identifiers][]"][value="${fileId}"]`);
            if (inputToRemove) {
                container.removeChild(inputToRemove);
                console.log(`[JS RemoveFile] Row ${rowIndex}: Removed hidden input for ID ${fileId}`);
            } else {
                console.warn(`[JS RemoveFile] Row ${rowIndex}: Hidden input for ID ${fileId} not found for removal.`);
            }
        });
    });
    ...
    function getLoopIndices(fromInputId, toInputId) {
        ...
        return { startIndex, endIndex }; // 返回计算好的索引范围
    }
    ...
    document.getElementById('apply-bulk-category').addEventListener('click', function() {
        ...
        for (let i = range.startIndex; i <= range.endIndex; i++) {
             const categorySelect = document.querySelector(`select[name="products[${i}][category_id]"]`);
             if (categorySelect) {
    ...
    document.getElementById('apply-bulk-min-qty').addEventListener('click', function() {
        ...
        for (let i = range.startIndex; i <= range.endIndex; i++) {
             const minQtyInput = document.querySelector(`input[name="products[${i}][min_order_quantity]"]`);
             if (minQtyInput) {
