            ...
            <div class="mb-6">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6"
                            ...
                            @foreach ($product->images as $image)
                                <div class="relative group existing-image-item" id="image-{{ $image->id }}"
                                    data-image-id="{{ $image->id }}">
                                    <img src="{{ asset('storage/' . $image->image_path) }}"
                                        alt="{{ __('admin.product_image') }}" class="h-24 w-24 object-cover rounded-lg">
                                    @if ($image->is_main)
                                        <span
                                            class="absolute top-0 left-0 bg-blue-500 text-white text-xs px-1 py-0.5 rounded-tl-lg rounded-br-lg main-image-badge">{{ __('admin.main_image') }}</span>
                                    @else
                                        <span
                                            class="absolute top-0 left-0 bg-blue-500 text-white text-xs px-1 py-0.5 rounded-tl-lg rounded-br-lg main-image-badge hidden">{{ __('admin.main_image') }}</span>
                                    @endif

                                    <!-- 设置主图 Radio -->
                                    <div class="absolute bottom-1 left-1" title="{{ __('admin.set_as_main') }}">
                                        <input type="radio" name="main_image_id" id="main_image_{{ $image->id }}"
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-lg p-6" id="dropzone">
                    ...
                    <div id="image-preview"
                        class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 mt-4"></div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function deleteImage(imageId, button) {
                ...
                // 构建URL - 确保使用正确的路由名称
                const url =
                    `{{ route('admin.products.images.destroy', ['product' => $product->id, 'image' => '_ID_']) }}`
                    .replace('_ID_', imageId);
