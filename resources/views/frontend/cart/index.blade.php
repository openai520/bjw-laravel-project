        ...
        <div x-data="shoppingCartData(
                ...
                <ul role="list" class="">
                    <template x-if="Object.keys(cartItems).length > 0">
                        <template x-for="(item, itemId) in cartItems" :key="itemId">
                            <li x-if="item" class="bg-white shadow overflow-hidden mb-4" style="border-radius: 20px !important;">
                                ...
                                <div class="grid grid-cols-6 sm:grid-cols-12 gap-x-4 sm:gap-x-6 items-center">
                                    ...
                                    <div class="col-span-2 sm:col-span-2">
                                        <div class="aspect-square bg-gray-100 rounded overflow-hidden">
                                            <img :src="item.main_image_url" :alt="item.name" class="w-full h-full object-cover object-center">
                                        </div>
                                    </div>
                                        <!-- Info & Actions Container -->
                                        <div class="col-span-4 sm:col-span-10 flex flex-col items-stretch space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                                            <!-- Title div -->
                                            <div class="min-w-0 sm:mr-6">
                                                <p class="text-xs sm:text-sm font-medium text-gray-900 leading-normal truncate" x-text="item.name"></p>
                                        </div>

                                            <!-- Controls Group div -->
                                            <div class="flex flex-wrap items-center justify-start gap-x-4 gap-y-2 sm:flex-nowrap sm:justify-start">
                                                <!-- Quantity div -->
                                                ...
                                                <!-- Subtotal div -->
                                                <div>
                                                    <p class="text-base font-extrabold text-red-600">¥<span x-text="(item.quantity * item.price).toFixed(2)"></span></p>
                                        </div>

                                                <!-- Delete div -->
                                                <div>
                                                    <button type="button" @click="itemToRemove = itemId" class="text-gray-400 hover:text-red-500 p-1 sm:p-2">
                        ...
                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                            <div class="space-y-2">
                                ...
                                <div class="flex items-baseline">
                                    ...
                                    <span class="ml-2 text-2xl font-bold text-red-600">¥<span x-text="totalAmount.toFixed(2)"></span></span>
                                </div>
                            </div>
                            <button type="button"
                                        ...
                                        <form @submit.prevent="submitInquiryForm" class="mt-5 space-y-4">
                                            ...
                                            <div>
                                                <label for="message" class="block text-sm font-medium text-gray-700">
                                                    {{ __('messages.message') }} <span class="text-gray-400 text-xs">({{ __('messages.optional') }})</span>
                                                </label>
                                                <textarea
                                                       name="message"
                                                       id="message"
            ...
            <div x-show="showInquirySuccessModal"
                ...
                <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full p-6 text-center">
...
<script>
    ...
    function shoppingCartData(initialCart, initialTotal, updateUrlTemplate, removeUrlTemplate, csrfToken, inquiryStoreUrl) {
        ...
        return {
            ...
            updateCartItemQuantity(itemId, newQuantity) {
                ...
                window.dispatchEvent(new CustomEvent('cart-updated', {
                    detail: { count: this.totalQuantity }
                }));
            ...
            removeCartItem(itemId) {
                ...
                // 保存要删除的商品以便回滚
                const tempItem = { ...this.cartItems[itemId] };
