...
<nav x-data="{ openSearch: false }" class="bg-white shadow">
    <div class="flex items-center">
        <a href="{{ route('frontend.products.index', ['lang' => app()->getLocale()]) }}" class="text-2xl font-bold text-gray-800">
            KALALA
        </a>
    </div>
    
    {{-- 大屏幕搜索框 --}}
    <form action="{{ route('frontend.products.index', ['lang' => app()->getLocale()]) }}" method="GET" class="hidden sm:flex flex-1 max-w-lg">
    ...
    {{-- 小屏幕搜索按钮 --}}
    <button type="button" @click.prevent="openSearch = !openSearch" class="sm:hidden p-2 text-gray-500 hover:text-gray-600">
