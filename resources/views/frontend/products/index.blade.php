...
@section('content')
    <main>
        ...
        <div class="w-full px-4 py-6"
            ...
            <!-- 产品网格容器 - 响应式优化 -->
            <div id="product-list-grid" class="grid gap-4"
            ...
            <!-- 骨架屏占位符（在没有更多内容时显示） -->
            <div x-show="loading" class="grid gap-4 mt-4"
                style="grid-template-columns: repeat(auto-fill, minmax(280px, 1fr));">
            ...
            Alpine.data('infiniteScrollHandler', (initialNextPageUrl, categorySlug = '') => ({
                ...
                init() {
                    if (!this.nextPageUrl) {
                        ...
                        console.log(
                            "InfiniteScroll: Initial nextPageUrl is empty. No more pages from the start."
                            );
                ...
                setupInfiniteScroll() {
                    ...
                    this.observer = new IntersectionObserver((entries) => {
                        entries.forEach(entry => {
                            if (entry.isIntersecting && !this.loading && !this
                                .noMorePages && this.nextPageUrl) {
                                console.log(
                                    "Loading indicator is intersecting! Triggering loadMore"
                                    );
                                this.loadMore();
                            }
                        });
                    ...
                    if (loadingIndicator) {
                        ...
                        console.error(
                            "Failed to find .loading-indicator to observe for infinite scroll.");
                    }
                },

                loadMore() {
                    console.log("loadMore called. Current nextPageUrl:", this.nextPageUrl, "Loading:",
                        this.loading, "NoMorePages:", this.noMorePages);

                    // 检查节流条件
                    const currentTime = Date.now();
                    if (currentTime - this.lastRequestTime < this.throttleDelay) {
                        console.log("loadMore: Request throttled. Last request was", currentTime - this
                            .lastRequestTime, "ms ago");
                        return;
                    }

                    if (this.loading || this.noMorePages || !this.nextPageUrl) {
                        console.log("loadMore returned early. Loading:", this.loading, "NoMorePages:",
                            this.noMorePages, "Has nextPageUrl:", !!this.nextPageUrl);
                        if (!this.nextPageUrl && !this
                            .noMorePages) { // Ensure noMorePages is set if URL is missing
                            this.noMorePages = true;
                        }
                        return;
                    }
                    ...
                    fetch(this.nextPageUrl, {
                        ...
                        .then(response => {
                            console.log("loadMore: Fetch response received. Status:", response
                                .status);
                            if (!response.ok) {
                        ...
                        .then(data => {
                            ...
                            console.log("loadMore: Found new products in data.html:", newProducts
                                .length);

                            const container = document.getElementById('product-list-grid');
                            if (!container) {
                                console.error(
                                    "loadMore: Could not find product-list-grid container!");
                                this.loading = false;
                                return;
                            }
                            ...
                            console.log("loadMore: Extracted next_page_url from JSON data:", this
                                .nextPageUrl);

                            if (!this.nextPageUrl) {
                                console.log(
                                    "loadMore: No next page URL in JSON data. Stopping infinite scroll."
                                    );
                                this.noMorePages = true;
                            }
